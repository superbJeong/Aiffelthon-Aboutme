...
import numpy as np
import dlib
from needs.wing import align_faces, FaceAligner
import argparse
from PIL import Image
from torchvision import transforms
import torchvision.utils as vutils

def cropping(i, img_org):
  try:
...
if __name__ == '__main__':
  ...
  for i, img_file in enumerate(file_list):
    ...
    try:
      ...
      img_cropped = cropping(i, img_org)
      ...
      save_image(x_aligned, 1, filename=os.path.join(dir_cropped, '_'.join(img_file.split(' '))))

      cnt += 1
      print(f'{i}번 째 이미지 완료 ({cnt} / {i})')
      print('------------------------------------------------------------------')
    except:
